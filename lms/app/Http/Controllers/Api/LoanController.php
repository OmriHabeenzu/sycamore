...
class LoanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Enforce 3× savings limit
        $savingsBalance = SavingsAccount::where('borrower_id', $validated['borrower_id'])
            ->where('company_id', $companyId)
            ->sum('balance');
        $maxAllowed = $savingsBalance * 3;

        if ($savingsBalance > 0 && $validated['principal_amount'] > $maxAllowed) {
            return response()->json([
                'message' => "Loan amount exceeds the 3× savings limit. Member's savings: K" .
                    number_format($savingsBalance, 2) . ". Maximum allowed: K" .
